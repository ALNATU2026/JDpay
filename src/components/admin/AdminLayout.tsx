...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const adminNavItems = [
    ...
    { label: 'Commissions (1.8% - 2%)', path: '/admin/commissions', icon: Percent },
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col md:flex-row text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-950 text-slate-300 border-r border-slate-800 shrink-0 select-none">
        ...
        <div className="p-3 border-t border-slate-800 space-y-1">
          <button
            onClick={onExitAdmin}
            className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-blue-300 hover:bg-blue-950/40 rounded-xl transition-colors cursor-pointer"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Admin Top Bar */}
        <header className="h-18 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-0 z-30 shadow-2xs transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-semibold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/50 px-2 py-0.5 rounded border border-purple-200 dark:border-purple-800">
                System Administration
              </span>
              <span className="hidden sm:inline text-xs font-bold text-slate-800 dark:text-slate-200 ml-2 capitalize">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Live MongoDB Status */}
            <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-[11px] font-semibold text-emerald-800 dark:text-emerald-300" title="Connected to MongoDB Atlas">
            ...
            <button
              onClick={onExitAdmin}
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors cursor-pointer"
            >
