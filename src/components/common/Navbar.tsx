    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between">
        ...
        <div className="flex items-center">
          ...
          >
            <img
              src={jdpayLogoEmblem}
              alt="JDpay Logo"
              className="w-9 h-9 rounded-xl object-contain shadow-xs border border-slate-200 p-0.5 group-hover:scale-105 transition-transform"
        ...
        <div className="hidden md:flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-[11px] font-semibold text-emerald-800" title="Connected to MongoDB Atlas: jdpay-cluster.ieezqvb.mongodb.net">
          ...
          {currentUser ? (
            <div className="flex items-center gap-3">
              {isMegaSuperAdminUser(currentUser) ? (
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-linear-to-r from-amber-500/10 via-purple-500/10 to-indigo-500/10 border border-amber-400/50 text-[11px] font-extrabold text-amber-900 shadow-xs">
              ...
              {isAnyAdminUser(currentUser) ? (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-lg transition-all shadow-xs ${
                    isMegaSuperAdminUser(currentUser)
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white bg-slate-900 rounded-lg hover:bg-blue-600 transition-all shadow-xs"
              >
                Go to Dashboard
            ...
            <>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4.5 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-all shadow-xs"
              >
                Create Account
              </button>
            </>
        ...
        <div className="md:hidden flex items-center gap-2">
          {currentUser && (
            ...
            >
              {isMegaSuperAdminUser(currentUser) ? 'Admin Portal' : 'Dashboard'}
            </button>
          )}
          <button
            ...
            {currentUser ? (
              <>
                <div className="px-3 py-1.5 text-xs text-slate-500">
                  Signed in as <span className="font-semibold text-slate-800">{currentUser.fullName}</span>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => handleLinkClick('/login')}
                  className="w-full py-2 text-xs font-semibold text-slate-800 bg-slate-100 rounded-lg text-center hover:bg-slate-200"
                >
                  Login
                </button>
                <button
